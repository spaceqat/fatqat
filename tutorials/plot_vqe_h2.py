...
energy of a Hamiltonian :math:`H` by making the variational principle
:math:`E(\theta) \geq E_0` do the work: a parameterized circuit prepares
:math:`|\psi(\theta)\rangle`, the energy
...
:math:`\theta` downhill. The lowest energy found is an upper bound on the
true ground-state energy :math:`E_0`.
...
`arXiv:1512.06860 <https://arxiv.org/abs/1512.06860>`_). The Hamiltonian
...
# qubit 0*: ``IZ`` acts with :math:`Z` on qubit 1.
#
...
# single term :math:`Z_0 Z_1`, while two entries would sum. Every
...
# two-qubit state — so this ansatz can reach :math:`E_0` in principle.
...
# :math:`\sigma_E = \sqrt{\sum_i c_i^2 \sigma_i^2}` — the scale of the
...
# ``RY`` (:math:`p = 0.01`) and, more strongly, after every ``CX``
