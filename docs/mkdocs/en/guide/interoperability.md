...
`QuantumCircuit` into a [`Program`][fatqat.Program], or appear inside Qiskit as
a backend.
...
of being approximated. See the [OpenQASM API reference](../api/interoperability/openqasm.md) for supported statements, version-specific
...
Use [`circuit_to_program`][fatqat.qiskit.circuit_to_program] when you want to cross the
...
Use [`FatqatBackend`][fatqat.qiskit.FatqatBackend] instead when the surrounding
...
The [Qiskit API reference](../api/interoperability/qiskit.md) documents the
