...
Use the general-purpose {py:class}`~fatqat.simulator.Simulator` to study
logical circuit evolution without choosing a hardware profile or Hamiltonian.
It applies the operations in a
{py:class}`~fatqat.Program` as written: it does not transpile, route, or attach
device timing.
...
{py:meth}`~fatqat.simulator.Simulator.run_sweep` binds each row of values to
...
```{eval-rst}
.. plot::
   :alt: Probability of measuring one follows a smooth sine-squared curve as the RY angle is swept from zero to two pi.
...
forms, see the {doc}`Simulator API <../api/simulator>`. Next, [Ask questions of
