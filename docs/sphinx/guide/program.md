...
{py:class}`~fatqat.Program` is the object you build and pass to every FatQat
...
A {py:class}`~fatqat.GridRegister` adds logical row and column selections:
...
{doc}`operations API reference <../api/operations>` lists the complete
...
A {py:class}`~fatqat.Parameter` stands in for a numeric operation argument.
...
{py:class}`~fatqat.ParameterVector` for an explicitly ordered group. The
...
dimensions they accept, so use the {doc}`qudit operation reference
<../api/operations/qudit-gates>` when you move beyond `Shift`.
...
{py:class}`~fatqat.operations.PulseOperation` cannot be represented by this
circuit renderer.
:::
...
the {doc}`Program API <../api/program>` when you need exact accepted forms or
validation behavior.
