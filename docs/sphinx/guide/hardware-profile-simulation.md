...
{py:class}`~fatqat.simulator.Simulator`, it evolves discrete gates at circuit
...
{py:class}`~fatqat.errors.UnsupportedOperationError`. FatQat does not silently
...
{py:class}`~fatqat.NoiseModel` of your own when loading, loss, or other
effects belong in the experiment.

{py:class}`~fatqat.simulator.SCQubitIBMSimulator` follows the same workflow
...
```{eval-rst}
.. plot::
   :alt: Two occupied atoms begin separated, move together when Pair applies with depolarizing noise, remain paired for CZ, and separate again when Unpair applies with depolarizing noise.
   :filename-prefix: atom-pairing-lifecycle
...
remove either atom. Use {py:class}`~fatqat.noise.Loss` instead when movement
...
{doc}`hardware-profile API <../api/simulators/index>`. Continue to
{doc}`Hamiltonian emulation <hamiltonian-emulation>` when pulse duration,
