...
| An operation is unsupported | Start with {py:class}`~fatqat.simulator.Simulator`. A constrained target such as {py:class}`~fatqat.simulator.SCQubitIBMSimulator` or {py:class}`~fatqat.simulator.AtomGridSimulator` intentionally accepts only its own native gate set and connectivity. |
