...
:::{dropdown} An integer target is ambiguous
:open:
...
:::{dropdown} Measurement reports a dimension mismatch
...
:::{dropdown} The Program still contains an unbound parameter
...
:::{dropdown} An operation works on Simulator but is unsupported here
:open:
...
:::{dropdown} A two-resource gate fails because of placement

The operation can be native while its two physical locations are not
connected. Inspect or choose a {py:class}`~fatqat.ResourceLayout` and keep the
...
:::{dropdown} A direct control is rejected by an emulator
...
:::{dropdown} Counts change each time
...
:::{dropdown} A statevector is unavailable for a noisy or dynamic run
...
:::{dropdown} The first Numba run is unexpectedly slow
...
:::{dropdown} A Result accessor says its data is unavailable
:open:
...
:::{dropdown} Count strings appear reversed
