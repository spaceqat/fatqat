...
| An operation is unsupported | Start with {py:class}`~fatqat.simulator.Simulator`. A ``Fake...Backend`` intentionally accepts only its own native gate set and connectivity. |
