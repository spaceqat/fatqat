...
A FatQat emulator still accepts a {py:class}`~fatqat.Program`, but it changes
...
that order as a {py:class}`~fatqat.emulator.SampledWaveform`, a
{py:class}`~fatqat.emulator.PulseControl`, and finally a
{py:class}`~fatqat.operations.PulseOperation`:
...
{py:class}`~fatqat.ResourceLayout` does not remap that address. The model and
