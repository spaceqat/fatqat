...
```{eval-rst}
.. plot::
   :alt: Logarithmic curves show statevector and density-matrix entry counts growing faster for qutrits than for qubits as subsystem count increases.
...
the {doc}`Simulator API <../api/simulator>` for exact method and result
constraints.
...
{doc}`Simulator runtime and execution <../api/simulator>`. Fusion is opt-in,
...
Simulator timing. The {doc}`emulator API <../api/emulators/index>` lists the
