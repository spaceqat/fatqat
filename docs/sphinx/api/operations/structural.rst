...
``RegisterView``. Reset is non-unitary: a statevector backend samples the
reset branch when a target is entangled, while a density-matrix backend
applies the deterministic reset channel. It has no attached-noise realization,
so using it as the ``operation=`` selector in
