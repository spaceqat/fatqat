Atom-array operations
=====================

.. currentmodule:: fatqat.operations

``Put`` manages atom occupancy; ``Pair`` and ``Unpair`` manage connectivity.
They are not unitary matrix gates. Add them with
:py:meth:`fatqat.Program.add`. :py:class:`~fatqat.simulator.AtomArraySimulator`
is the only built-in backend that implements them. Other matrix and pulse
backends raise :py:exc:`~fatqat.errors.UnsupportedOperationError`.

.. list-table:: Atom-array operations
   :header-rows: 1
   :widths: 14 17 30 19 20

   * - Value
     - Scalar targets
     - Effect
     - Conditions
     - Attached noise
   * - :py:data:`Put`
     - One or more
     - Loads ``|0>`` into each empty site; leaves occupied sites unchanged.
     - Allowed.
     - :py:class:`~fatqat.noise.Loss` only, after each enabled ``Put`` operation.
   * - :py:data:`Pair`
     - Exactly two
     - Adds their undirected connectivity edge; repeated pairing is a no-op.
     - Rejected.
     - :py:class:`~fatqat.noise.Loss` or a supported finite channel.
   * - :py:data:`Unpair`
     - Exactly two
     - Removes their edge; removing an absent edge is a no-op.
     - Rejected.
     - :py:class:`~fatqat.noise.Loss` or a supported finite channel.

If a program contains ``Put``, every declared site starts empty for every shot.
Sites are populated only when ``Put`` runs, and a later ``Put`` can reload a
lost atom. A :py:class:`~fatqat.noise.Loss` declaration attached to ``Put``
shares the operation's condition and runs after every matching ``Put``
operation whose condition passes, even when the site was already occupied and
the ``Put`` itself did nothing.

``Pair`` and ``Unpair`` update the connectivity used by later supported gates;
they do not change the quantum state or make an unsupported gate available. In
the built-in atom-array profile, :py:data:`CZ` is native and requires a current
pairing. The atom backend rejects a condition on either instruction with
:py:exc:`~fatqat.errors.BackendValidationError` when the program runs.

.. autodata:: fatqat.operations.Put
   :no-value:
.. autodata:: fatqat.operations.Pair
   :no-value:
.. autodata:: fatqat.operations.Unpair
   :no-value:
