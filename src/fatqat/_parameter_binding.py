...
def _normalize_parameter_mapping(
    ...
    expanded: dict[Parameter, Real] = {}

    def add(parameter: Parameter, raw_value: object) -> None:
        if parameter in expanded:
            raise ValueError(f"parameter {parameter.name!r} is assigned more than once")
        expanded[parameter] = _validate_parameter_scalar(raw_value)
    ...
    for key, raw_value in values.items():
        ...
        if isinstance(key, ParameterVector):
            ...
            vector_values = _materialize_vector_value(raw_value)
