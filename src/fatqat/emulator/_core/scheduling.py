...
from .target import ResourceClaim
...
def _resource_claims_conflict(
    ...
    """Return whether two blocks reserve an identical target resource claim.

    Claims are opaque to the scheduler: it only ever intersects two blocks'
    claim sets, so a target may partition its hardware however it likes.
    """
