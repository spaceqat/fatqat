...
from .._backends.engine_contract import _SimulationConfig
    ...
    def __post_init__(self) -> None:
        ...
        if self.parallel_mode == "auto":
            ...
            raise BackendValidationError(
                "Emulator v0.1 supports only parallel_mode='auto' or 'serial'"
            )
        if self.max_workers not in (None, 1):
            raise BackendValidationError(
                "Emulator v0.1 supports only max_workers=None or 1"
            )
        object.__setattr__(
