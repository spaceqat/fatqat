...
class PulseOperation(Operation):
    ...
    channel address against its physical model. For a ``TransmonModel``, drive
    and detuning channels bind one declared subsystem; an exchange channel
    binds two subsystems and their declared coupling. The emulator also
    validates model-family compatibility, resource names, duration, waveform,
    amplitude, and concurrency constraints at this stage.
    ...
    another compatible model. The matrix ``Simulator`` does not support direct
    pulse operations.

    ``program.add(operation, condition=...)`` may guard the block. A false
    condition disables its controls, but the full block duration still elapses
    under model drift and background Lindblad sources. Operation-scoped noise
    cannot be attached to a direct pulse operation.
