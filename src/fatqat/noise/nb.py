...
typed arrays the fused per-shot kernel in ``simulator.engine.nb`` walks.

Division of labour with ``simulator.engine.nb``, and why it runs this way: *applying*
...
one-directional - ``simulator.engine.nb`` imports this module, never the reverse.
...
...``), exactly as ``fatqat.simulator.engine.nb`` is treated.

Conventions match ``simulator.engine.np`` / ``simulator.engine.nb``: little-endian flat
...
def _inverse_cdf_pick(
    ...
    ``simulator.engine.nb`` has the same two steps for basis-index sampling. It is
    duplicated rather than imported: this module must not import from
    ``simulator.engine.nb`` (see the module docstring on the forced direction).
    """
