"""Tests _SCQubitRotationSimulator: native gates, arbitrary topology, and noise."""
...
def test_sc_rotation_profile_map_exposes_native_device_operands_for():
    m = _SCQubitRotationSimulator().implementation_map
...
def test_sc_rotation_profile_accepts_arbitrary_couplings_and_exposes_sites():
    backend = _SCQubitRotationSimulator(num_qubits=5, couplings=COUPLINGS)
...
def test_sc_rotation_profile_map_rx_ry_rz_are_uniform():
    m = _SCQubitRotationSimulator().implementation_map
...
def test_sc_rotation_profile_map_iswap_and_cz_have_no_class_keyed_rule():
...
def test_sc_rotation_profile_map_rejects_cx_and_sx():
    m = _SCQubitRotationSimulator().implementation_map
...
def test_fake_backend_rejects_non_native_operation_families():
    ...
    # SCQubitSimulator - none of the three are legal here.
...
def test_fake_backend_rejects_more_than_sixteen_qubits():
    ...
    with pytest.raises(
        BackendValidationError, match="_SCQubitRotationSimulator supports at most 16"
    ):
        _SCQubitRotationSimulator().run(
            p, result_config={"counts": False, "final_state": True}
        )
...
def test_fake_backend_rejects_non_qubit_dimension_registers():
    ...
    with pytest.raises(
        ...
        match="_SCQubitRotationSimulator only supports qubit dimensions",
    ):
        _SCQubitRotationSimulator().run(
            p, result_config={"counts": False, "final_state": True}
        )
...
def test_grid_register_is_flattened_on_arbitrary_topology():
    ...
    p = Program([qubits])
    backend = _SCQubitRotationSimulator(num_qubits=6, couplings=((0, 1),))
...
def test_default_noise_model_is_fully_supported():
    model = _SCQubitRotationSimulator.default_noise_model()
    assert _SCQubitRotationSimulator().validate_noise_model(model) is None
...
def test_noisy_backend_leaks_errors_but_stays_mostly_correct():
    backend = _SCQubitRotationSimulator(
        noise=_SCQubitRotationSimulator.default_noise_model()
    )
    shots = 4000
    counts = (
...
def test_rz_carries_relaxation_like_other_rotations():
    backend = _SCQubitRotationSimulator(
        noise=_SCQubitRotationSimulator.default_noise_model()
    )
    program = Program(1)
...
def test_default_noise_model_is_a_fresh_extensible_model():
    first = _SCQubitRotationSimulator.default_noise_model()
    second = _SCQubitRotationSimulator.default_noise_model()
