...
def test_counts_default_with_measurement():
    ...
    p.add(ops.X, 0)
    p.measure(0, 0)
    result = Simulator("DM").run(p, shots=10, simulation_config={"seed": 0}).result()
...
def test_density_matrix_default_attached_when_no_measurement():
    p = Program(1)
    p.add(ops.H, 0)
    job = Simulator("DM").run(p, result_config={"counts": False})
...
def test_statevector_accessor_unavailable_on_density_matrix_result():
    p = Program(1)
    p.add(ops.H, 0)
    result = Simulator("DM").run(p, result_config={"counts": False}).result()
    with pytest.raises(ResultFieldUnavailableError):
...
def test_reset_program_counts_are_deterministic_for_any_shots():
    ...
    p.add(ops.X, 0)
    p.measure(0, 0)
    result = Simulator("DM").run(p, shots=50, simulation_config={"seed": 3}).result()
...
def test_run_rejects_unknown_result_config_keys():
    ...
    with pytest.raises(BackendValidationError, match="does not support result_config"):
        Simulator("DM").run(p, result_config={"counts": False, "statevector": True})
...
def test_run_rejects_unknown_simulation_config_keys():
    with pytest.raises(
        ...
        Simulator("DM").run(Program(1), simulation_config={"gpu": True})
...
# --- target-aware resolution (mirrors Simulator, see test_resolution.py) --
...
def test_target_aware_map_allows_registered_target_key():
    ...
    backend = Simulator("DM", implementation_map=m)
